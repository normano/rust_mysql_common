...
pub struct BinlogEventHeader {
    ...
    pub timestamp: u32,
    /// Raw event Type.
    pub event_type: RawField<u8, EventType>,
...
pub struct BinlogEventFooter {
    ...
    pub checksum_alg: Option<RawField<u8, BinlogChecksumAlg>>,
}
...
pub struct FormatDescriptionEvent {
    ...
    pub binlog_version: RawField<u16, BinlogVersion>,
...
pub struct ExecuteLoadQueryEvent {
    ...
    pub dup_handling: RawField<u8, LoadDuplicateHandling>,
}
...
pub struct IntvarEvent {
    ...
    pub subtype: RawField<u8, IntvarEventType>,
    pub value: u64,
}
...
pub struct UserVarEvent {
    ...
    pub is_null: bool,
    /// Type of a value.
    pub value_type: RawField<i8, ItemResult>,
...
pub struct IncidentEvent {
    pub incident_type: RawField<u16, IncidentType>,
    pub message: RawText,
}
...
pub struct OptionalMetadataIter<'a> {
    columns: &'a RawSeq<u8, ColumnType>,
    data: &'a [u8],
}

impl<'a> OptionalMetadataIter<'a> {
    /// Reads type-length-value value.
    fn read_tlv(&mut self) -> io::Result<(RawField<u8, OptionalMetadataFieldType>, &'a [u8])> {
    ...
    fn next_tlv(
        &mut self,
    ) -> Option<io::Result<(RawField<u8, OptionalMetadataFieldType>, &'a [u8])>> {
                ...
                Ok(t) => match t {
                    SIGNEDNESS => {
                        ...
                        match v.len().cmp(&num_flags_bytes) {
                            ...
                            Greater => return Err(Error::new(Other, "bytes remaining on stream")),
                            _ => (),
                        }
                        let mut flags = BitVec::from_slice(v);
                        flags.truncate(num_numeric);
                        Ok(OptionalMetadataField::Signedness(flags))
                    }
                    DEFAULT_CHARSET => {
                        let default_charset = v.read_lenenc_int()? as u16;
                        let mut non_default = HashMap::new();
                        while !v.is_empty() {
                            let index = v.read_lenenc_int()?;
                            let charset = v.read_lenenc_int()? as u16;
                    ...
                    COLUMN_CHARSET => {
                        ...
                        for _ in 0..num_columns {
                            columns.push(v.read_lenenc_int()? as u16);
                        }
                    ...
                    COLUMN_NAME => {
                        ...
                        for _ in 0..num_columns {
                            columns.push(RawText(read_lenenc_str!(&mut v)?.into()));
                        }
                    ...
                    SET_STR_VALUE => {
                        ...
                        for _ in 0..num_columns {
                            let num_variants = v.read_lenenc_int()? as usize;
                            let mut names = Vec::with_capacity(num_variants);
                            for _ in 0..num_variants {
                                names.push(RawText(read_lenenc_str!(&mut v)?.into()));
                            }
                            columns.push(names);
                        }
                    ...
                    GEOMETRY_TYPE => {
                        ...
                        for _ in 0..num_columns {
                            columns.push(RawField::new(v.read_lenenc_int()? as u8))
                        }
                    ...
                    SIMPLE_PRIMARY_KEY => {
                        ...
                        while !v.is_empty() {
                            indexes.push(v.read_lenenc_int()?);
                        }
...
pub struct TableMapEvent {
    ...
    pub columns_type: RawSeq<u8, ColumnType>,
...
impl BinlogStruct for TableMapEvent {
    ...
    fn write<T: Write>(&self, version: BinlogVersion, mut output: T) -> io::Result<()> {
        ...
        output.write_all(self.null_bitmask.as_slice())?;
    ...
    fn len(&self, version: BinlogVersion) -> usize {
        ...
        len += S(1);
        len += S(crate::misc::lenenc_int_len(self.get_columns_count() as u64));
        len += S(self.get_columns_count());
        len += S(crate::misc::lenenc_bytes_len(&self.columns_metadata));
...
impl RowsEvent {
    ...
    pub fn len(&self, version: BinlogVersion) -> usize {
        ...
        len += S(crate::misc::lenenc_int_len(self.num_columns)); // number of columns
        let bitmap_len = (self.num_columns as usize + 7) / 8;
        ...
        fn read_row<T: Read>(
            ...
            for i in 0..(num_columns as usize) {
                ...
                if cols.get(i).copied().unwrap_or(false) {
                    ...
                    columns.push(column.into_owned());

                    // check if column is null
                    if null_bitmap.get(image_idx).copied().unwrap_or(true) {
                        values.push(Value::NULL);
                    } else {
                        let column_meta = tme.get_column_metadata(i).unwrap_or(&[]);
                        values.push(read_binlog_value(
                            &mut input,
                            column_type,
                            column_meta,
                            signedness[i],
...
impl BinlogStruct for GtidEvent {
    ...
    fn len(&self, version: BinlogVersion) -> usize {
        ...
        len += S(crate::misc::lenenc_int_len(self.tx_length as u64)); // tx_length
    ...
    fn binlog_event_roundtrip() -> io::Result<()> {
        ...
        'outer: for file_path in binlogs {
            ...
            while let Some(ev) = binlog_file.next() {
                ...
                if let EventData::TableMapEvent(ref ev) = event {
                    table_map_event_map.insert(ev.table_id, ev.clone().into());
                    dbg!(ev);
                }

                match &event {
                    EventData::WriteRowsEventV1(ev) => {
                        dbg!(ev);
                        let tme = table_map_event_map.get(&ev.0.table_id).unwrap();
                        for event_row in ev.0.rows(tme) {
                            dbg!(event_row.unwrap());
                        }
                    }
                    EventData::WriteRowsEvent(ev) => {
                        dbg!(ev);
                        let tme = table_map_event_map.get(&ev.0.table_id).unwrap();
                        for event_row in ev.0.rows(tme) {
                            dbg!(event_row.unwrap());
                    ...
                    EventData::UpdateRowsEventV1(ev) => {
                        dbg!(ev);
                        let tme = table_map_event_map.get(&ev.0.table_id).unwrap();
                        for event_row in ev.0.rows(tme) {
                            dbg!(event_row.unwrap());
                    ...
                    EventData::DeleteRowsEventV1(ev) => {
                        dbg!(ev);
                        let tme = table_map_event_map.get(&ev.0.table_id).unwrap();
                        for event_row in ev.0.rows(tme) {
                            dbg!(event_row.unwrap());
