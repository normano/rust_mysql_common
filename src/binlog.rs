...
impl UserVarEvent {
    ...
    pub fn get_type(&self) -> Result<ItemResult, i8> {
        match ItemResult::try_from(self.value_type) {
            val @ Ok(ItemResult::STRING_RESULT) => val,
            val @ Ok(ItemResult::REAL_RESULT) => val,
            val @ Ok(ItemResult::INT_RESULT) => val,
            val @ Ok(ItemResult::DECIMAL_RESULT) => val,
...
impl TableMapEvent {
    ...
    pub fn get_column_type(&self, col_idx: usize) -> Option<Result<ColumnType, u8>> {
        self.columns_type
            .get(col_idx)
