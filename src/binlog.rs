...
use crate::{
    constants::{ColumnType, ItemResult},
...
#[repr(transparent)]
pub struct RawField<T, U: TryFrom<T, Error = T>>(pub T, PhantomData<U>);

impl<T: Copy, U: TryFrom<T, Error = T>> RawField<T, U> {
    ...
    pub fn get(&self) -> Result<U, T> {
        U::try_from(self.0)
    }
}

impl<T: fmt::Debug, U: fmt::Debug> fmt::Debug for RawField<T, U>
where
    T: Copy,
    U: TryFrom<T, Error = T>,
{
...
#[repr(transparent)]
pub struct RawSeq<T: Copy, U: TryFrom<T, Error = T>>(pub Vec<T>, PhantomData<U>);

impl<T: Copy, U: TryFrom<T, Error = T>> RawSeq<T, U> {
    /// Creates a new wrapper.
    ...
    pub fn get(&self, index: usize) -> Option<Result<U, T>> {
        self.0.get(index).copied().map(U::try_from)
    }
...
impl<T: fmt::Debug, U: fmt::Debug> fmt::Debug for RawSeq<T, U>
where
    T: Copy,
    U: TryFrom<T, Error = T>,
{
